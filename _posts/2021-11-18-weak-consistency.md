...
讲到弱一致性，不得不提到最终一致性，因为这是很多系统采用的一致性模型。
...
假设有三个服务节点：A、B和C，其中A是primary，B和C是secondary。

1. 客户端向A发送请求`incr x`

2. A收到请求，本地执行完，然后返回给客户端执行成功

3. A将请求同步给C，消息刚发出去A就挂了

4. 然后将B提升为新的primary，此时又有客户端向B发送请求`incr x`

5. B执行完，并向C节点同步请求

6. 对于C节点，分别收到了来自A和来自B的`incr x`操作，其本地x+1执行两次，而此时的primary B只执行x+1一次，出现数据的不一致

- 不满足交换律的情况

还是相同的情况，A是primary，B和C是secondary。
...
4. 然后将B提升为新的primary，此时又有客户端向B发送请求`set x 2`

5. B执行完，向C同步请求

6. 对于C节点，先收到了来自B的请求，后收到了来自A的请求。最终导致x = 1。此时与primary B中的x = 2不同，出现数据的不一致。

- 不满足结合律的情况

最后说说不满足结合律的情况，由于此时满足了交换律，所以：

`a + (b + c) = (b + c) + a`

因此，如果不满足`a + b + c = (b + c) + a`，则表示不满足结合律。
...
4. 然后将B提升为新的primary，此时又有客户端向B接连发送请求`set x 2`和`set x 3`

5. B执行完，向C同步请求

6. 对于C节点，先接收到了`set x 2`和`set x 3`，后收到了来自A的请求`set x 1`。最终导致C中的x=1，与B中的x=3不同，出现数据的不一致

## 开源实现

下面讲解几个实现了弱一致性的几个开源实现，其实他们实现的都是最终一致性。
...
另外前面讲到，最终一致模型可能会带来冲突问题，所以何时解决冲突是需要考虑的一个问题。这里有两个选择：写时解决和读时解决。写时解决是大部分系统所采取的方案，在这种方案中，读流程会比较简单。不过在这种系统中，如果写入不能到达全部或者大部分副本，写入将会被拒绝（如果只写入一个节点，冲突永远不会被发现）由于Dynamo需要保证永远可写，因此冲突解决是由读来完成的。
...
- 为每个副本分配一个唯一ID，通过指定的规则，例如，序列号大的副本写入有效

- 以某种方式将这些值合并在一起。例如，按字母顺序排序，然后拼接在一起

- 利用预定义好的格式来记录和保留冲突相关的所有信息，然后依靠应用层的逻辑解决冲突。
...
- 当一个节点知道D1和D2，在它收到D4和它的vector clock后，可以断定D1和D2已经被新数据覆盖了，因此可以安全对D1和D2进行垃圾回收，这也就是syntatic reconciliation过程，不需要应用参与解决。
