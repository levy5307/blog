...
最后一个是***C***，其代表一致性。其定义是比较模糊的：数据库必须保证事务的执行从一个一致性状态转移到另一个一致性状态。其主要是指保证事务操作后保持原有的数据的约束。比如转账事务完成后，转账的双方的总金额不能多页不能少。在ACID中，C是目的，AID是手段，为了达到C的目的而提供的手段。数据库必须实现了AID的这三个特性才有可能实现C。
