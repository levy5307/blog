...
- Client只关注其自己的输入，对于Word内部执行的是什么命令、Client输入怎么转换成的命令、调用file哪个接口，这些应该是系统内部的实现，客户端是完全不关心的，应该对客户端进行屏蔽。如果实在想不通，想象一下Word是一个独立的服务，客户端通过网络对其进行连接，并向Word服务发送输入信息。至于Word内部的信息，客户端不应该知道。
