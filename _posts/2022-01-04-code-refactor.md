...
Visitor模式是一个相对比较难理解的设计模式，本人阅读了很多讲解Visitor的文章，很多都将重点聚焦在将数据数据持有者和数据访问者分离，这会令很多人很疑惑：为什么要把本来在元素内部实现的、关于元素自身行为的逻辑抽取到访问者上。其实，Visitor模式的核心思想是：

将不同的逻辑耦合到一个类上，会导致随着场景的变迁，该类越来越膨胀。这时通过Visitor模式，将一些方法移出去创建一个vistor，使得不同场景的代码得到解耦。（语法树很多采用Visitor实现）
