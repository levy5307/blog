...
- 再往上，PairLattice是Anna提供的另一种结构，其key是一个代表vector clock，value是对应与Key的数据，也是一个ValueLattice结构（merge方法由用户提供）。对于两个PairLattice P(a, b)和Q(a, b)，如果P.a > Q.a，那么P(a, b)在因果上follow Q(a, b)，那么写入结果则是P(a, b)，否则就是Q(a, b)。如果P.a和Q.a无法比较，则需要对PairLattice的key和value分别采用merge函数进行合并。
