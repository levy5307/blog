...
由于读未提交隔离级别的性能读已提交并没有什么太大的区别，所以实际中很少使用读未提交。
...
读提交的问题在于，只要事务提交，其所做的数据修改就可以看到。可重复读在这一点对其进行了优化，解决了不可重复读的问题。

常采用的实现方法叫做MVCC（多版本并发控制）。其每个事务都从数据库的一致性快照中读取，事务看到的所有数据都是最近提交的数据，即使随后数据可能会被修改。这样就可以避免不可重复读的问题。

其与读已提交的不同在于，对于读已提交，只需要保存数据的两个版本就够了，事务提交前读取旧版本数据，提交后则读取新版本数据。
...
需要注意的一点是，可重复读并不要求解决幻读的问题（***幻读***是不可重复读的一种特殊场景, 要求范围数据的可重复读，关注两次查询语句执行得到的结果集不同）。

MVCC其实已经解决了脏读、不可重复读以及幻读的问题，从这一点来看已经符合串行化的定义。但是却与事务的真正串行执行结果是不同的，从这一点来看又不符合串行化。这里主要是ANSI的各种定义的不严谨造成的，所以在这里将MVCC还是划归为可重复读级别（很多DBMS把MVCC称为串行化隔离，比如OceanBase）。

另外，上面提到了ANSI对于这些定义不够严谨，所以也不要过于纠结，知道怎么回事就可以了。
...
<<<<<<< HEAD
=======
需要说明的一点是，这里的写更新实际上是由幻读导致的。***幻读***是指，在一个事务中的写入改变了另一个事务查询结果的现象。对于读-读场景中的幻读，快照级别隔离得到了解决。但是对于如上读-写场景中的幻读（导致了写倾斜）问题，快照级别隔离却无能为力。

另外，这里说明一下，之前在网上查资料的时候发现很多人对于幻读和不可重复读之间分不清。其实我认为这两个完全不是一个维度的概念，不可重复读（又名读倾斜）是幻读所导致的一个问题，就和写倾斜一样（看名字也是相对应的：读倾斜、写倾斜）。只不过是不可重复读是发生在读-读场景下，而写倾斜是发生在读-写场景下。在快照级别隔离下，只是解决了读-读场景下的幻读问题（即不可重复读），并没有完全解决幻读的问题。

很多文章认为不可重复读是针对数据的修改，幻读是对区间的增删，我认为这样是不对的，因为如果是这样的话，MVCC完全可以解决幻读问题了。

>>>>>>> 11a374fd313ab629d7090f9d4f16eab7e16c0924
串行化可以用来解决上述问题。
