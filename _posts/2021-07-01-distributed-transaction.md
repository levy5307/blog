...
可重复读要求不能有脏读、不可重复读，但是可以存在幻读（***幻读***是不可重复读的一种特殊场景, 要求范围数据的可重复读，关注两次查询语句执行得到的结果集不同）。其常采用的实现方法叫做MVCC（多版本并发控制）。其每个事务都从数据库的一致性快照中读取，事务看到的所有数据都是最近提交的数据，即使随后数据可能会被修改。这样就可以避免不可重复读的问题。
...
虽然可重复读并不要求解决幻读的问题，但是MVCC其实已经解决了脏读、不可重复读以及幻读的问题，从这一点来看已经符合串行化的定义。但是却与事务的真正串行执行结果是不同的（更新丢失，write skew问题），从这一点来看又不符合串行化。这里主要是ANSI的各种定义的不严谨造成的，所以在这里将MVCC还是划归为可重复读级别（很多DBMS把MVCC称为串行化隔离，比如OceanBase）。
