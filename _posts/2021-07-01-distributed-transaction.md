...
MVCC其实已经解决了脏读、不可重复读以及幻读的问题，从这一点来看已经符合串行化的定义。但是却与事务的真正串行执行结果是不同的（更新丢失，write skew问题），从这一点来看又不符合串行化。这里主要是ANSI的各种定义的不严谨造成的，所以在这里将MVCC还是划归为可重复读级别（很多DBMS把MVCC称为串行化隔离，比如OceanBase）。
