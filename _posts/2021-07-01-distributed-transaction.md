...
另外，这里说明一下，之前在网上查资料的时候发现很多人对于幻读和不可重复读之间分不清。其实我认为这两个完全不是一个维度的概念，不可重复读是幻读所导致的一个问题，就和写倾斜一样。只不过是不可重复读是发生在读-读场景下，而写倾斜是发生在读-写场景下。在快照级别隔离下，只是解决了读-读场景下的幻读问题（即不可重复读），并没有完全解决幻读的问题。
