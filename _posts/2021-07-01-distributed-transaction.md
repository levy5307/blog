...
对于上面写倾斜的例子，事务是基于查询的数据决定接下来的动作，当查询的时候条件成立，而在提交之前，由于其他写事务更改了数据，导致先前成立的条件现在已经不成立了，因此导致写倾斜问题的发生。在SSI中要避免这种问题，就是要通过检测发现这种对查询结果进行变化的写事务，并使其中止回滚。

那么数据库如何知道查询结果是否发生了变化呢？可以分下面两种情况：

- 读取是否作用于一个过期或者即将过期的MVCC对象（读取之前已经有未提交的写入）

- 检查写入是否影响即将完成的读取（读取之后又有新的写入）
