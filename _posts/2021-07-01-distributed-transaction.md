...
读提交的问题在于，只要事务提交，其所做的数据修改就可以看到，因此会有不可重复读的问题。可重复读要求不能有脏读、不可重复读，但是可以存在幻读（***幻读***是不可重复读的一种特殊场景, 要求范围数据的可重复读，关注两次查询语句执行得到的结果集不同）。
...
另外，上面提到了ANSI对于这些定义不够严谨，存在一些冲突的地方，所以也不要过于纠结，知道怎么回事就可以了。
