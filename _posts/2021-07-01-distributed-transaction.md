...
在ANSI SQL92标准中，所有的隔离级别都是使用锁来实现的。但是目前业内普遍使用较多的实现方式是MVCC（MVCC通常被定义为快照隔离级别，但是由于ANSI SQL92标准中不包含快照隔离级别，所以这里不在讲述快照隔离级别。而快照级别隔离与ANSI SQL92中的可重复读更为接近，所以这里将MVCC归为可重复读）
