...
application data model位于directory的上层。一个应用会在创建一个或者多个database。每个database可以包含无穷数量的结构化table。spanner中的table和关系数据库的表类似，具有行、列和版本值。我们不会详细介绍spanner的查询语言，它看起来很像SQL，只是针对protocol-buffer的支持做了一些扩展。
