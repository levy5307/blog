...
spanner将客户端作为2pc发起者，其选择一个参与者作为协调者。具体流程如下图：

![](../images/spanner-2pc.png)

从图中我们可以看到，spanner中client感受到的提交延迟为***1次rpc，1次参与者写日志和1次协调者写日志***。但是由于spanner中都是长事务，所以采用了悲观的并发控制，即2PL并发控制。所以在2pc的过程中会有加锁的操作，对2pc性能会有一定影响。
