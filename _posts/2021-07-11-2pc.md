...
这里提一点，其实虽然primary和secondary的日志复制是同步的，但是提交操作却异步的，secondary的提交在下一次的prepare req到来之时进行提交。这里可以对其进行***优化***：添加一个新的commit rpc，在prepare阶段之后，由primary向secondary发送，secondary收到该请求后进行commit操作，这样便可以免去secondary commit操作对2pc的延时。
