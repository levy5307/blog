...
所以，PacificA由于减少了等待commit过程，将客户端感受到的提交延迟降低为***2次协调者写日志、1次参与者写日志以及1次rpc往返***。
...
这里提一点，其实虽然primary和secondary的日志复制是同步的，但是提交操作却异步的，secondary的提交在下一次的prepare req到来之时进行提交。这里有两种优化方案：

1. 可以优化将secondary的提交在一个后台线程去操作，这样免去secondary的提交操作对2pc延时的影响。

2. 添加一个新的commit rpc，在prepare阶段之后，由primary向secondary发送，secondary收到该请求后进行commit操作
