...
这里提一点，其实虽然primary和secondary的日志复制是同步的，但是提交操作却异步的，secondary的提交在下一次的prepare req到来之时进行提交。这里可以对其进行***优化***：在将该prepare resp返回之后，再对上次2pc的secondary进行提交，这样可以降低写的延时和长尾。
